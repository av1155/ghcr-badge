...
function extractDownloadStats(html: string) {
    ...
    const totalDownloadsMatch = html.match(
        /Total downloads[\s\S]{0,500}?<h3\b([^>]*)>([^<]+)<\/h3>/i,
    );
...
async function fetchPackageStats(urls: string[]) {
    const results = await Promise.all(
        urls.map(async (url) => {
            try {
                ...
                return { success: true as const, url, ...stats };
            } catch (error) {
                return {
                    success: false as const,
                    url,
    ...
    const hasRawCounts = successfulResults.some(
        (result) => result.downloadCountRaw !== null,
    );
    const summedRawCount = successfulResults.reduce(
        (sum, result) => sum + (result.downloadCountRaw ?? 0),
        0,
    );
...
async function handleApiRequest(c: Context<{ Bindings: Env }>) {
    const { owner, repo, pkg } = c.req.param() as {
        owner: string;
        repo?: string;
        pkg: string;
    };
    ...
    try {
        const { downloadCount, downloadCountRaw, urls } = await fetchPackageStats([
            githubUrl,
        ]);
        ...
        const result = {
            downloadCount,
            downloadCountRaw,
            repo: { url: urls[0], urls, owner, repo: repo ?? null, package: pkg },
        ...
        return c.json(
            {
                repo: { url: githubUrl, owner, repo: repo ?? null, package: pkg },
                success: false,
